...
#######################################
z.df <- wgs %>% mutate(variant = case_when(is.na(VariantofInterest) ~ "not_sequenced",
                                           VariantofInterest=="VOC-20DEC-01" ~ "alpha",
                                           VariantofInterest=="VOC-21APR-02" ~ "delta",
                                           TRUE ~ "other")) 

z <- z.df %>% group_by(Collection_Date) %>% dplyr::summarise(N=n(),
                                                             alpha=sum(variant=="alpha"), 
                                                             delta=sum(variant=="delta")) %>% 
  mutate(p_delta = delta/N*100,  p_alpha=alpha/N*100) %>% 
  dplyr::select(Collection_Date, p_delta, p_alpha) %>% 
  pivot_longer(cols=p_delta:p_alpha) %>% 
  mutate(name=gsub("p_","",name))

z %>% ggplot(aes(x=Collection_Date, y=value, colour=name)) + geom_point() +
  labs(x="Date", y="Percentage", colour="Variant", title="")
