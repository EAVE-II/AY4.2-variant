...
z.df <- z.df %>%  filter(specimen_date <= as.Date("2021-08-16")) # remove people with test dat later than nejm letter

z.df.fit <- z.df %>%  filter(In_Hosp_At_Test==0 & lab=="lh" & in_eave==1)

table(z.df.fit$s_gene,z.df.fit$covid_death, exclude=NULL)
table(z.df.fit$vt_phe ,z.df.fit$covid_death, exclude=NULL)
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   n_risk_gps   + vt_phe "))

z.fit <- coxph(fmla.final , data=z.df.fit)
summary(z.fit)

#adjustment for age is important

z.df.fit %>% group_by(vt_phe) %>% dplyr::summarise(mean_age = mean(age_year), sd_age = sd(age_year))
round(prop.table(table(z.df.fit$vt_phe, z.df.fit$n_risk_gps, exclude=NULL), 1)*100,1)

z.df.fit <- z.df %>%  filter(In_Hosp_At_Test==0 & lab=="lh" & in_eave==1 & s_gene %in% c("S_Neg","S_Pos")) %>% 
  mutate(age_gp = cut(age_year, breaks = c(0,39,59,120), labels=c("16-39","40-59","60+")))
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   vt_phe "))
z.agg <- pyears(fmla.final, data=z.df.fit, data.frame=TRUE)$data %>% mutate(rate=event/pyears*100)
