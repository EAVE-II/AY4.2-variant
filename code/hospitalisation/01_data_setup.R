...
#       filter(!duplicated(EAVE_LINKNO)) %>%
#       dplyr::select(-EAVE_CHRONIC_PANCREATITIS, -EAVE_PREGNANCY, -EAVE_TRANSPLANT)
...
cdw_full  <- readRDS(paste0(Location,"EAVE/GPanalysis/data/CDW_deduped.rds")) %>%
             ...
             arrange(EAVE_LINKNO, SpecimenDate, desc(result)) %>%
...
z <- Positive_Tests %>%  
     mutate(days = as.numeric(SpecimenDate - a_begin)) %>% 
     mutate(test_before_begin = cut(days, breaks = c((min(days)-1), -28, -21, -14, -7, 0, max(days)),
                                labels=c("1+m", "4w","3w","2w","0-6d","post-start")))

df_cohort <- df_cohort %>% 
             left_join(dplyr::select(z, EAVE_LINKNO, test_before_begin), by="EAVE_LINKNO") %>%
...
df_cohort <- df_cohort %>% 
             mutate(ew = if_else(EAVE_LINKNO %in% z_ids, 1, eave_weight*(z_N - z_k)/(z_N - z_m)) ) %>%
             dplyr::select(-eave_weight) %>% 
             dplyr::rename(eave_weight=ew)

# # Get datazones
# datazones <- readRDS("/conf/EAVE/GPanalysis/data/EAVE_demographics_SK.rds") %>%
#               select(EAVE_LINKNO, DataZone) %>%
#               left_join(read_csv(paste0(Location,"/EAVE/GPanalysis/data/restored/map_files/Datazone2011Lookup.csv")) %>% 
#                           dplyr::select(DataZone, InterZone, Council, HB))

# df_cohort <- df_cohort %>% 
#              left_join(datazones, by="EAVE_LINKNO") %>% 
...
########## 2 Create study datatset of all who were virally sequenced ########################

# Gather data on all people who have been virally sequenced.
df_seq <- wgs %>% 
      left_join( cdw_full %>% select(EAVE_LINKNO, SpecimenDate, test_result_record_source),
                 by = c( 'Collection_Date' = 'SpecimenDate', 'EAVE_LINKNO' = 'EAVE_LINKNO')) %>%
      ...
      mutate(lab = if_else(test_result_record_source == "ECOSS", "nhs","lh"),
             variant = case_when(is.na(VariantofInterest) ~ "not_sequenced",
                             ...
                             TRUE ~ "other")) %>%
      select(EAVE_LINKNO, Collection_Date, lineage, variant, lab) %>%
      # Combine with cohort data
      left_join(df_cohort, by="EAVE_LINKNO") %>%
      # Add all deaths
...
z <- df_seq %>% left_join( all_hospitalisations %>% 
          ...
          mutate(admission_date = case_when( discharge_date <= Collection_Date ~ as.Date(NA),
                                     TRUE ~ admission_date),
                 emergency = case_when( discharge_date <= Collection_Date ~ NA,
                                     TRUE ~ emergency),
                 discharge_date = case_when( discharge_date <= Collection_Date ~ as.Date(NA),
                                     TRUE ~  discharge_date))
...
df_seq <- df_seq %>%        
      mutate(
        ...
        days = as.numeric(Collection_Date -min(Collection_Date, na.rm = TRUE)),
        death = if_else(is.na(NRS.Date.Death), 0L, 1L),
        vs1 = case_when(is.na(date_vacc_1) | date_vacc_1 > Collection_Date ~ "uv",
                       date_vacc_1 <= Collection_Date &  date_vacc_1 > Collection_Date - 28 ~ "v1_0:27",
                       TRUE ~ "v1_28+"),
        vs2 = case_when(is.na(date_vacc_2) | date_vacc_2 > Collection_Date ~ "uv",
                       date_vacc_2 <= Collection_Date &  date_vacc_2 > Collection_Date - 28 ~ "v2_0:27",
                       TRUE ~ "v2_28+"),
        vs = if_else(vs2=="uv", vs1, vs2), 
        time_to_hosp = if_else(!is.na(admission_date), 
                              as.numeric(admission_date - Collection_Date), 
                              as.numeric(a_end - Collection_Date)) ,
        time_to_death = if_else(!is.na(NRS.Date.Death), 
                               as.numeric(NRS.Date.Death - Collection_Date), 
                               as.numeric(a_end - Collection_Date))      ) %>% 
      mutate(
        ...
        hosp_covid = case_when( (!is.na(admission_date) & (admission_date - Collection_Date <= 14)) ~1,
                                        TRUE ~ 0),
        death_covid = case_when(covid_death_cert == 1 | 
                (!is.na(NRS.Date.Death) & (NRS.Date.Death - Collection_Date <= 28)) ~1,
                                                                TRUE ~ 0),
...
df_seq <- df_seq %>% left_join(covid_icu_death %>% dplyr::select(-SpecimenDate) %>% 
  ...
  mutate(covid_icu_death = if_else(is.na(date_icu_death), 0L, 1L),
         time_to_icu_death = if_else(is.na(date_icu_death), as.numeric(a_end - Collection_Date),
                                     as.numeric(date_icu_death - Collection_Date))) %>% 
...
# Filter out age less than 18 and inconsistent vaccination records
df_seq <- filter(df_seq, ageYear >= 18 & flag_incon == 0)
